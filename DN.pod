...
=head1 SYNOPSIS
  ...
  $dn = X500::DN->ParseRFC2253 ('cn=John Doe, o=Acme\\, Inc., c=US') or die;
  print $dn->getRFC2253String(), "\n";

  $dn = new X500::DN (new X500::RDN ('c'=>'US'), new X500::RDN ('cn'=>'John Doe'));
  $rdn0 = $dn->getRDN (0);
  $c = $rdn0->getAttributeValue ('c');
...
=item * $object = X500::DN->ParseOpenSSL ('/C=US/CN=John Doe');

Creates a DN object from an openssl formatted DN string notation.
...
Returns the DN as a string formatted according to one of openssl's syntaxes.
...
=item * Due to Parse::RecDescent's greedyness, multivalued RDNs are not parsed correctly. It might be possible to work around this.

=back

=head1 AUTHOR

Robert Joop E<lt>yaph-020416@timesink.deE<gt>
